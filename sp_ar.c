...
static sa_index_t steps_to_find[4][3] = {
    {1, 2, 3},
    {2, 3, 0},
    {3, 1, 0},
    {2, 1, 0}
...
find_empty(register sa_table* table, register sa_index_t pos)
{
    ...
    sa_entry *entry;
    sa_index_t *steps = steps_to_find[pos & 3];
    pos &= FLOOR_TO_4;
    entry = table->entries+pos;

    if (entry[steps[0]].next == SA_EMPTY) { new_pos = pos + steps[0]; goto check; }
    if (entry[steps[1]].next == SA_EMPTY) { new_pos = pos + steps[1]; goto check; }
    if (entry[steps[2]].next == SA_EMPTY) { new_pos = pos + steps[2]; goto check; }
